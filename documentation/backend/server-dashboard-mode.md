...
| `rangeClause` | `rangeFilterMatch` | `coalesce(col, 0)`: a real 0 is treated like NULL and gated by the `includeNull` flag. Bounds are cast `::double precision` so a wide-open max (`Number.MAX_SAFE_INTEGER`) does not overflow int4 columns. |
